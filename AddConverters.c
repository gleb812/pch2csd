...
int AddConverters(void)
{
    ...
    */
    ...
    for(CableCounter=0;CableCounter<CCa;CableCounter++)
    {
        ...
        ItempModule=aIOTable[CableCounter][4]; // Модуль назначения
        ItempPort=aIOTable[CableCounter][5]; // Порт назначения
        ...
        // предположим, что тип провода установлен верно, а значит проверка на то, что источник соответствует типу провода, является избыточной!

        // Откытие файла входов выходов для модуля назначения
    ...
    for(CableCounter=0;CableCounter<CCk;CableCounter++)
    {
        ...
        ItempModule=kIOTable[CableCounter][4]; // Модуль назначения
        ItempPort=kIOTable[CableCounter][5]; // Порт назначения
